...
export default function HomePage() {
  ...
  const { metar, taf, metarHistoryMaxTemp, metarHistoryByDays, loading: aviationLoading, error: aviationError } =
    useAviationWeather(selectedCity?.icao ?? null, selectedCity?.timezone ?? 'UTC')
  ...
  const windDir =
    dayIndex === 0
      ...
      : (forecast?.daily?.wind_direction_10m_dominant?.[dayIndex] ?? null)
  const windSpeed =
    dayIndex === 0
      ? (forecast?.current?.wind_speed_10m ?? null)
      : (forecast?.daily?.wind_speed_10m_max?.[dayIndex] ?? null)
  const precipitation =
    dayIndex === 0
