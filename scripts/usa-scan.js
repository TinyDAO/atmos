#!/usr/bin/env node
/**
 * Scan US cities (NWS forecast) vs Polymarket highest-temperature bins (YES odds).
 * Run: node --experimental-strip-types scripts/usa-scan.js
 *   or: pnpm run usa-scan
 */
import { getUsaWeather } from './usaweather.util.js'
  ...
  try {
    return await fetch(url, {
      ...
      headers: {
        ...
        'User-Agent': 'weather-usa-scan-cli/1.0',
        ...init.headers,
      },
    })
  } finally {
...
function printHelp() {
  console.log(`
用法: node --experimental-strip-types scripts/usa-scan.js [选项]

选项:
  --city=id1,id2   只扫描指定城市 id（如 nyc,los-angeles）
  --no-color       禁用 ANSI 颜色
  --help, -h       显示帮助
...
说明:
  天气来自 NOAA NWS MapClick（°F）；与 Polymarket 档位对比时换算为 °C。

示例:
  pnpm run usa-scan
  pnpm run usa-scan -- --city=nyc
`)
}
 ...
 * @param {number | null} p.refMaxC  NWS 日最高温（°C），用于最近档位
 */
function printMarketTable({ rows, closestIdx, refMaxC }) {
  ...
  console.log(`    ${dim(top)}`)
  console.log(`    ${dim(cell('档位', 'YES', 'Δ(档−NWS°C)', '成交量'))}`)
  console.log(`    ${dim(mid)}`)
...
async function main() {
  ...
  for (const city of cities) {
    ...
    console.log(dim(`  NWS: ${city.latitude}, ${city.longitude}`))

    let weather
    try {
      weather = await getUsaWeather(city.latitude, city.longitude)
    } catch (e) {
      stats.weatherErrors++
      console.log(`  ${yellow('NWS 天气')} ${yellow('失败')}: ${e instanceof Error ? e.message : e}`)
      console.log()
      continue
    }
    ...
    for (const { index: dayIndex, key, label: dayZh } of DAY_LABELS) {
      ...
      const maxF = dayForecast?.maxTemp
      ...
      if (maxF == null || !Number.isFinite(maxF) || nwsMaxC == null) {
        console.log(`    ${yellow('NWS 预测最高温')} —`)
      } else {
        console.log(
          `    ${magenta('★ NWS 预测最高温')}: ${bold(String(maxF))}°F (${bold(nwsMaxC.toFixed(1))}°C)`
        )
      }
      ...
      const closestIdx = indexOfClosestBin(visible, nwsMaxC ?? NaN)
      printMarketTable({ rows: visible, closestIdx, refMaxC: nwsMaxC })
      if (closestIdx >= 0 && nwsMaxC != null && Number.isFinite(nwsMaxC)) {
        console.log(dim(`    ★ = 与 NWS 最高温 (${nwsMaxC.toFixed(1)}°C) 最接近的档位`))
      }
    }
  ...
  console.log(bold('— 汇总 —'))
  console.log(
    `  城市: ${stats.cityRuns}/${stats.citiesTotal} · 天次: ${stats.dayRuns} · NWS 失败: ${stats.weatherErrors} · Gamma 请求失败: ${stats.gammaErrors} · 无事件: ${stats.noEvent} · 无档位: ${stats.noBinsParsed}`
  )
}
